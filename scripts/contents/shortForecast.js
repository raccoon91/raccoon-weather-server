...
const getForecast = async (location, forecastDate, forecastTime) => {
	const response = await axios.get(`http://newsky2.kma.go.kr/service/SecndSrtpdFrcstInfoService2/ForecastTimeData`, {
		params: {
...
module.exports = () => {
	...
	try {
		...
		console.log(`[short_forecast][SUCCESS][${forecastDate}${forecastTime}][${date.dateLog(moment.tz("Asia/Seoul"))}]`);
	} catch (err) {
		console.warn(
			`[short_forecast][FAIL][${err.message}][${forecastDate}${forecastTime}][${date.dateLog(
