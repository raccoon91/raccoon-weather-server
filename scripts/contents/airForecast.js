...
const saveAirForecast = async today => {
  const [pm10Forecast, pm25Forecast] = await axios.all([
    getAirForecast("PM10", today),
    getAirForecast("PM25", today)
  ]);
...
module.exports = () => {
  ...
  try {
    ...
    console.log(
      `[airForecast][SUCCESS][${today}][${date.dateLog(
        moment.tz("Asia/Seoul")
      )}]`
    );
  } catch (err) {
    console.error(
      `[airForecast][FAIL][${err.message}][${today}][${date.dateLog(
        moment.tz("Asia/Seoul")
      )}]`
    );
