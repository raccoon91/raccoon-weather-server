...
const saveAirpollution = async () => {
	const [pm10, pm25] = await axios.all([getAirpollution("PM10"), getAirpollution("PM25")]);
...
module.exports = () => {
	...
	try {
		...
		console.log(`[airpollution][SUCCESS][${today}][${date.dateLog(moment.tz("Asia/Seoul"))}]`);
	} catch (err) {
		console.error(`[airpollution][FAIL][${err.message}][${today}][${date.dateLog(moment.tz("Asia/Seoul"))}]`);
