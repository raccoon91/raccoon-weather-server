...
const getCurrentWeather = async (location, currentDate, currentTime) => {
	const response = await axios.get(`http://newsky2.kma.go.kr/service/SecndSrtpdFrcstInfoService2/ForecastGrib`, {
		params: {
...
const saveWeather = async (currentDate, currentTime, yesterday) => {
	const response = await axios.all(
		locationList.map((location) => getCurrentWeather(location, currentDate, currentTime)),
	);
...
module.exports = async () => {
	const { currentDate, currentTime, yesterday } = getCurrentDate(moment().tz("Asia/Seoul"));

	try {
		await saveWeather(currentDate, currentTime, yesterday);
		console.log(`[weather][SUCCESS][${currentDate}${currentTime}][${date.dateLog(moment.tz("Asia/Seoul"))}]`);
	} catch (err) {
		console.error(
			`[weather][FAIL][${err.message}][${currentDate}${currentTime}][${date.dateLog(moment.tz("Asia/Seoul"))}]`,
		);
